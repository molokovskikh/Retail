...
namespace Retail.MicroORM
{
    ...
    public static class DbORM
    {
        ...
        /// <summary>
        /// Логер для сбора внештатных событий
        /// </summary>
        private static readonly ILog logger = LogManager.GetLogger(typeof(DbORM));
        ...
        static string _genQueryUpdate(object entity, string tableName = null)
        {
            ...
            logger.Info(string.Format("Проверка подготовленного запроса SQL:\n{0}", result));
            return result;
        }
                    ...
                    while (rd.Read()) //Получим все результаты выборки
                    {
                        ...
                        for (int f = 0; f < rd.FieldCount; f++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    entity = entity == null ? ((T)Activator.CreateInstance(typeof(T))) : entity;
                                    customHandler(entity, columnName, rd.GetValue(f));                                    
                                }
                            }
                        }
